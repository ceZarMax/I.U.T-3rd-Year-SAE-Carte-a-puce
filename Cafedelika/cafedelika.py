...
def enregistrer_transaction(montant, libelle, type_operation):
    try:
        connection = mysql.connector.connect(
            host='http://127.0.0.1/phpmyadmin/index.php',
            user='root',
            password='root',
        ...
        # Récupérer le nom et prénom de la carte
        print_nom()
        print_prenom()
        ...
        cursor.execute(select_etu_query, (nom, prenom))  # Assurez-vous d'avoir les valeurs correctes de nom et prénom
...
def debiter_carte(montant, libelle, type_operation):
    ...
    apdu = [0x82, 0x03, 0x00, 0x00, 0x02, 0x00, montant]

    try:
        data, sw1, sw2 = conn_reader.transmit(apdu)
        if sw1 == 0x90:
            # Enregistrez la transaction dans la base de données
            enregistrer_transaction(montant, libelle, type_operation, nom, prenom)
